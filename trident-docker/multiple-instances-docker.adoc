...
summary: 'Plusieurs instances de Trident sont nécessaires lorsque vous souhaitez disposer simultanément de plusieurs configurations de stockage.  La clé pour avoir plusieurs instances est de leur donner des noms différents en utilisant l"option --alias avec le plugin conteneurisé, ou l"option --volume-driver lors de l"instanciation de Trident sur l"hôte.' 
---
...
----
. Lancez la deuxième instance en spécifiant un alias et un fichier de configuration différents.
+
[source, console]
----
...
----
. Lancez le plugin avec une configuration iSCSI utilisant un ID de pilote personnalisé :
+
[source, console]
----
