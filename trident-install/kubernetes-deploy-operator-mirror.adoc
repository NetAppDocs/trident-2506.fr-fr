...
[role="lead"]
Vous pouvez déployer manuellement l'opérateur Trident pour installer Trident.  Ce processus s'applique aux installations où les images de conteneur requises par Trident sont stockées dans un registre privé.  Si vous ne disposez pas d'un registre d'images privé, utilisez lelink:kubernetes-deploy-operator.html["processus de déploiement standard"] .
...
.Avant de commencer
Connectez-vous à l'hôte Linux et vérifiez qu'il gère un système fonctionnel etlink:requirements.html["cluster Kubernetes pris en charge"^] et que vous disposez des privilèges nécessaires.
